...
class ThemeUpdate extends Command
{
    ...
    private function updateGitignoreFile(
        OutputInterface $output,
        string $themeDirectory,
        string $newThemeDirectory
    ) {
        ...
        if ($gitignoreContents === false) {
            $output->writeln([
                '<error>',
                'An error ocurred while reading the .gitignore file.',
                '</error>',
            ]);
        ...
        $gitignoreContents = preg_replace(
            "/!public\/content\/themes\/{$themeDirectory}/",
            "!public/content/themes/{$newThemeDirectory}",
            $gitignoreContents
        );

        try {
            $this->filesystem->dumpFile($gitignorePath, $gitignoreContents);
        } catch (IOException $e) {
            $output->writeln([
                '<error>',
                'An error ocurred while updating the .gitignore file.',
                $e->getMessage(),
                '</error>',
            ]);
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // update theme directory and .gitignore
        if (!empty($newThemeName)) {
            ...
            if (!$this->updateGitignoreFile($output, $themeDirectory, $newThemeDirectory)) {
                return;
            }
        }
